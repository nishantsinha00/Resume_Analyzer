...
class GetRelevantCvAPI(APIView):
    ...
    def post(self, request):
        ...
        output = []
        for i in range(min(request.data['nCV'], len(docs))):
            if  1 - docs[i][1] < 0.4:
                break
            ...
            for project in obj['projects']:
                ...
                project['relevancy'] = round(score*10)
            ...
            for professional_experience in obj['professional_experience']:
                ...
                score = cosine_similarity(emb_vec, jd_vec)
                professional_experience['relevancy'] = round(score*10)

            print('test')

            resp = json.dumps(obj, indent=4)

            output.append({"output" : resp, "score": 1 - docs[i][1]})
